...
export function ManageApiKeysContent() {
  ...
  return (
    <div className="space-y-4 pt-2">
      {error && (
        <div className="p-2 bg-red-500/10 backdrop-blur-sm border border-red-500/20 rounded-xl text-red-700 dark:text-red-400 text-xs">
          {error}
        </div>
      )}

      {createdKey && (
        <div className="p-3 bg-green-500/10 backdrop-blur-sm border border-green-500/20 rounded-xl">
          <h4 className="font-medium text-green-800 dark:text-green-300 text-sm mb-1">API Key Created</h4>
          <p className="text-xs text-green-700 dark:text-green-400 mb-2">
            Copy this key now. You won&apos;t see it again!
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 bg-white/10 backdrop-blur-sm p-1.5 rounded-lg border border-green-500/20 font-mono text-xs break-all">
              {createdKey}
            </code>
            <GlassButton 
              size="sm" 
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {apiKeys.map((key) => (
              <GlassCard key={key.id} size="sm" className="flex items-center justify-between p-2">
                <div>
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    <span>Prefix: <code className="bg-white/10 px-1 rounded">{key.prefix}</code></span>
          ...
          <div className="py-4 text-center text-muted-foreground border-2 border-dashed border-white/10 rounded-xl text-xs">
            No active API keys
          </div>
        )}
      </div>
