...
import { InsightConfig, GeneratedInsight } from '@prisma/client';
...
export class InsightConfigService {
   ...
   */
  private toConfigResponse(config: InsightConfig): InsightConfigResponseDto {
    return {
      id: config.id,
   ...
   */
  private toInsightResponse(insight: GeneratedInsight): GeneratedInsightResponseDto {
    return {
      id: insight.id,
      babyId: insight.babyId,
      type: insight.type as InsightType,
      content: insight.content as Record<string, unknown>,
  ...
  async getInsightHistory(
    ...
    const where: { babyId: string; type?: InsightType } = { babyId };
      ...
      try {
        ...
        await this.prisma.generatedInsight.create({
          data: {
            ...
            content: summary as object, // Cast to object to satisfy Prisma Json type
            periodStart: startDate,
            periodEnd: endDate,
          },
        });
